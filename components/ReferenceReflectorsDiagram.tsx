...
export function ReferenceReflectorsDiagram() {
  ...
  const H = (W * VB_H) / VB_W;
  const dim = "#79828D", ink = theme.ink;
  return (
    <View style={{ marginBottom: 16 }}>
      <Svg width={W} height={H} viewBox={`0 0 ${VB_W} ${VB_H}`}>
        ...
        <Rect x={40} y={66} width={305} height={84} fill="#232A33" stroke="#3A4350" strokeWidth={1.2} />
        <Circle cx={95} cy={112} r={7} fill={theme.bgPanel} stroke="#3A4350" strokeWidth={1} />
        <SvgText x={95} y={94} fill={ink} fontFamily={mono} fontSize={10} textAnchor="middle">SDH</SvgText>
        <Rect x={198} y={104} width={8} height={46} fill={theme.bgPanel} stroke="#3A4350" strokeWidth={1} />
        <SvgText x={202} y={98} fill={ink} fontFamily={mono} fontSize={10} textAnchor="middle">FBH</SvgText>
        <Rect x={288} y={66} width={8} height={16} fill={theme.bgPanel} stroke="#3A4350" strokeWidth={1} />
