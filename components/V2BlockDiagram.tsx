...
export function V2BlockDiagram() {
  ...
  const H = (W * VB_H) / VB_W;
  const dim = theme.muted, amber = theme.amber, amberT = theme.amberText, ink = theme.inkSoft;
  ...
  return (
    <View style={{ marginBottom: 16 }}>
      <Svg width={W} height={H} viewBox={`0 0 ${VB_W} ${VB_H}`}>
        ...
        <Polygon points="120,57 178,57 170,41 112,41" fill={theme.bgInput} stroke={theme.borderHi} strokeWidth={1} />
        <SvgText x={145} y={52} fill={ink} fontFamily={mono} fontSize={9} textAnchor="middle">probe</SvgText>

        <Circle cx={150} cy={107} r={3.3} fill={theme.bg} stroke={theme.borderHi} strokeWidth={1} />
